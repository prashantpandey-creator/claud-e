...
def _probe_prompt(n: Dict[str, Any]) -> str:
    return (
        ...
        "If it needs a fact FROM the Mumbai prod box, `meditate ssh-ro list` names the "
        "three read-only questions you can ask it directly (`meditate ssh-ro run <name>`) — "
        "raw ssh is not available to you, and this is not a workaround for that, it is the "
        "one door that was actually opened. If your question is not one of those three, "
        "it genuinely is not reachable from here; say so.\n"
